...
exports.GetAllBusiness = async (req,res) => {
	...
	if(requiredFields == "") {
		...
		var condition = {
			...
			attributes: {
				exclude: ['is_deleted','is_enable','auth_token','device_type',
	...
	try {
		...
		promises.push(
			await giftCardModel.findAll({
				where: {
					isDeleted: false,status: true,deleted_at: null,expire_at: {
						[Op.gt]: currentDate
					},
				},include: [
					{
			...
			await cashbackModel.findAll({
				where: {
					isDeleted: false,status: true,deleted_at: null,validity_for: {
						[Op.gt]: currentDate
					},
				},
				include: [
				...
				order: [['createdAt','DESC']],
				attributes: ['id','title','cashback_value','product_category_id','product_id','description','validity_for','createdAt']
			...
			await discountModel.findAll({
				...
				order: [['createdAt','DESC']],
				attributes: ['id','business_id','title','discount_type','discount_value','product_category_id','product_id','validity_for','status','createdAt']
			...
			await couponeModel.findAll({
				...
				order: [['createdAt','DESC']],
				attributes: ['id','business_id','title','coupon_code','coupon_type','product_category_id','product_id','value_type','coupon_value','validity_for','expire_at','description','status','createdAt']
			...
			await loyaltyPointModel.findAll({
				where: {
					isDeleted: false,status: true,deleted_at: null,validity: {
						[Op.gt]: currentDate
					},
				},include: [
					{
				...
				order: [['createdAt','DESC']],
				attributes: ['id','business_id','loyalty_type','name','points_earned','product_id','amount','points_redeemed','validity','validity_period','status','createdAt']
		...
		if(requiredFields == "") {
			...
			promises.push(
				...
				cashbackModel.findAll({
					where: {
						isDeleted: false,status: true,business_id: data.business_id,deleted_at: null,validity_for: {
							[Op.gt]: currentDate
						},
					}
				}).then(async CashbackData => {
				...
				couponeModel.findAll({
					where: {
						isDeleted: false,status: true,business_id: data.business_id,deleted_at: null,expire_at: {
							[Op.gt]: currentDate
						},
					}
				}).then(async CouponeData => {
...
function mergeRandomArrayObjects(arrays) {
	...
	return mergedArray.sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));
}
// Rewards List END
		...
		if((paramType) && !(typeArr.includes(paramType))) {
			...
			if(paramType == 'gift_cards') {
				...
				cashbackModel.findOne({
					where: {
						id: data.id,status: true,isDeleted: false,validity_for: {
							[Op.gt]: currentDate
						},
					}
				}).then(async cashbackData => {
				...
				discountModel.findOne({
					where: {
						id: data.id,status: true,isDeleted: false,deleted_at: null,validity_for: {
							[Op.gt]: currentDate
						},
					}
				}).then(async discountData => {
				...
				couponeModel.findOne({
					where: {
						id: data.id,status: true,isDeleted: false,deleted_at: null,expire_at: {
							[Op.gt]: currentDate
						},
					}
				}).then(async couponeData => {
		...
		if(data) {
			if(requiredFields == "") {
				...
				if(data.event_id && validation == true) {
					...
					condition.where = {
						...condition.where,...{
							id: data.event_id,is_deleted: false,end_date: {
								[Op.gt]: currentDate
							},
						}
					}
