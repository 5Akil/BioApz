...
exports.makeEntry = async (req, res) => {
  try {
    ...
    if (requiredFields == '') {
      ...
      } else if (loyaltyCard.no_of_tokens - 1 == loyaltyCardHistory?.count) {

        const exist = await loyaltyTokenFreeProductModel.findOne({ where: { status: true, is_deleted: false, business_id: data.business_id, loyalty_token_card_id: data.loyalty_token_card_id } })
        if (_.isEmpty(exist)) {
          const body = {
            user_id: user.id,
            business_id: data.business_id,
          ...
          await loyaltyTokenFreeProductModel.create(body)
        }
  ...
  try {
    ...
    if (requiredFields == '') {
      ...
      await followUnfollowBusinessModel.findAndCountAll(condition).then(async followedBusinessData => {
        console.log(followedBusinessData.rows);
