...
exports.list = async (req, res) => {
	try {
		...
		const condition = {
			...
			order: [['created_at', 'DESC']],
			offset: skip,
			limit: limit || undefined, // Set limit only if page_size is provided and not 0
		};
		const { count, rows: notificationList } = await notificationModel.findAndCountAll(condition);
		for (const data of notificationList) {
			data.dataValues.is_read = data.notification_receivers[0].is_read;
			delete data.dataValues.notification_receivers;
			delete data.dataValues.created_at;
		}
		const unreadNotificationCount = notificationList.reduce((count, data) => {
			if (!data.dataValues.is_read) {
				count++;
			}
			return count;
		}, 0);
...
exports.markAsRead = async (req, res) => {
	try {
		...
		if (_.isEmpty(requiredFields)) {
			...
			await notificationReceiverModel.update({is_read: true,},{where: condition.where});
...
exports.delete = async (req, res) => {
	try {
		...
		if (_.isEmpty(requiredFields)) {
			...
			await notificationReceiverModel.update({is_deleted: true,},{where: condition.where});
